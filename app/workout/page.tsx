...
export default function WorkoutPage() {
  ...
  const finishWorkout = async () => {
    ...
    if (sessionInsert.error || !sessionInsert.data) {
      setFeedback(`Could not save workout: ${sessionInsert.error?.message}`);
      return;
    }
      ...
      <div className="mx-auto w-full max-w-3xl rounded-2xl border border-zinc-800 bg-zinc-900 p-6">
        ...
        <div className="mt-5 space-y-3">
          {exerciseRows.map((exercise) => {
            ...
            return (
              <div key={exercise.id} className="rounded-lg border border-zinc-700 p-3">
                <p className="font-semibold">{exercise.exercises?.name ?? "Exercise"}</p>
                <p className="text-sm text-zinc-400">
